...
```ruby
# TODO: lambda 支持多个参数，多个语句自动 begin
# gem install sxp
require 'sxp'
$tv = 0
def fresh
...
def ftv(t)
  case t
  in :int | :bool then []
  in Symbol then [t] #  if t=~/^t\d+$/ 
  in [:'->', a, b] then ftv(a) | ftv(b)
  else []
  end
end
# 应用代换
def apply(s, t)
  # p [:apply,s,t]
  case t
  in Symbol then s.fetch(t, t)
  in [:'->', a, b] then [:'->', apply(s, a), apply(s, b)]
  else t
  end
end
# 组合代换 s1 ◦ s2
...
def infer(expr, env)
  ...
  in [:lambda, x, body]
    v = fresh
    t, s = infer(body, env.merge(x => { vars: [], type: v }))
    [[:'->', apply(s, v), t], s]
in [:let, x, e1, e2]
    t1, s1 = infer(e1, env)
    env1 = apply_env(s1, env)
    t2, s2 = infer(e2, env1.merge(x => generalize(env1, t1)))
    [t2, compose(s2, s1)]
in [e1, e2]
  t1, s1 = infer(e1, env)
  t2, s2 = infer(e2, apply_env(s1, env))
  v = fresh
  #p [:app, e1, e2]
  #p [t1, t2, s1, s2, apply(t1, s2)]
...
def pretty(t)
  case t
  in Symbol then t.to_s
  in [:'->', a, b] then "(#{pretty(a)} -> #{pretty(b)})"
  else t.to_s
  end
end
def run(code)
  expr = SXP.read(code)
  env = {}
  type = infer(expr, env)[0]
  puts "expr: #{expr}"
  puts "type: #{pretty(type)}"
  puts
end
# ------------------------------------------------------------------------------
# 测试

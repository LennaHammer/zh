...
def _branch(cond)
  return !!cond unless cond.is_a?(Z3::BoolExpr)

  $solver.push; $solver.assert(cond); true_sat = $solver.check == :sat; $solver.pop
  $solver.push; $solver.assert(cond.~); false_sat = $solver.check == :sat; $solver.pop

  return true  if true_sat && !false_sat
  return false if false_sat && !true_sat

  if $path.length > $pathcondition.length
    branch = $path[$pathcondition.length]
    decision = branch ? cond : cond.~
    $pathcondition << decision
    $solver.assert(decision)
    return branch
  end

  raise DepthError if $path.length >= 10
  $path << true
  $pathcondition << cond
  $solver.assert(cond)
  true
...
def _truthy(v) = v != false

def _z3_cmp(a, b, op)
  a, b = b, a if b.is_a?(Z3::ArithExpr) && !a.is_a?(Z3::ArithExpr)
  a.send(op, b)
end

BUILTINS = {
  :+       => ->(*a) { a.reduce(:+) },
  :-       => ->(a, *r) { r.empty? ? -a : [a, *r].reduce(:-) },
  :*       => ->(*a) { a.reduce(:*) },
  :"="     => ->(a, b) { _z3_cmp(a, b, :==) },
  :<       => ->(a, b) { _z3_cmp(a, b, :<) },
  :>       => ->(a, b) { _z3_cmp(a, b, :>) },
  :<=      => ->(a, b) { _z3_cmp(a, b, :<=) },
  :>=      => ->(a, b) { _z3_cmp(a, b, :>=) },
  :cons    => ->(a, b) { b.is_a?(Array) ? [a] + b : [a, b] },
  :car     => ->(l)  { l.is_a?(Array) && !l.empty? ? l[0] : nil },
  :cdr     => ->(l)  { l.is_a?(Array) && l.length > 1 ? l[1..] : [] },
  :"null?" => ->(l)  { l.is_a?(Array) && l.empty? },
  :"pair?" => ->(l)  { l.is_a?(Array) && l.length >= 1 },
  :list    => ->(*a) { a },
  :length  => ->(l)  { l.length },
  :append  => ->(*ls) { ls.reduce([], :concat) },
  :filter  => ->(fn, l) { l.select { |x| _branch(fn.call(x)) } },
  :map     => ->(fn, l) { l.map { |x| fn.call(x) } },
  :error   => ->(m) { raise m.to_s },
  :not     => ->(x) { x.is_a?(Z3::BoolExpr) ? x.~ : x == false },
  :"eq?"   => ->(a, b) { a.is_a?(Z3::ArithExpr) || b.is_a?(Z3::ArithExpr) ? a == b : a.equal?(b) },
  :"equal?" => ->(a, b) { a.is_a?(Z3::ArithExpr) || b.is_a?(Z3::ArithExpr) ? a == b : a.equal?(b) },
  :display => ->(x) { x },
}
...
class Lambda
  ...
  def call(*args)
    new_env = @env.dup
    @params.zip(args) { |p, a| new_env[p] = a }
    scheme_eval(new_env, @body)
  end
...
def _fresh_int(name = nil)
  if name
    Z3::Int(name.to_s)
  else
    $fresh_counter += 1
    Z3::Int("_fresh_#{$fresh_counter}")
  end
end
...
def scheme_eval(env, expr)
  ...
  in true | false
    expr
  in Symbol => sym
    return true  if sym == :"#t"
    return false if sym == :"#f"
    raise NameError, "Unbound variable: #{sym}" unless env.key?(sym)
    env[sym]
  in String | Integer
    expr
  in []
  ...
  in [:quote, val]
    val
  in [:if, cond, then_expr, else_expr]
    scheme_eval(env, _branch(scheme_eval(env, cond)) ? then_expr : else_expr)
  in [:define, Array => lhs, rhs]
    env[lhs[0]] = Lambda.new(lhs[1..], rhs, env)
    env[lhs[0]]
  in [:define, Symbol => var, val]
    env[var] = scheme_eval(env, val)
    env[var]
  in [:lambda, params, body]
    Lambda.new(params, body, env)
  in [:begin, *exprs]
    exprs.reduce(nil) { |_, e| scheme_eval(env, e) }
  in [:let, bindings, body]
    new_env = env.dup
    bindings.each { |b| new_env[b[0]] = scheme_eval(env, b[1]) }
    scheme_eval(new_env, body)
  in [:and, *exprs]
    r = true
    exprs.each { |e| r = scheme_eval(env, e); return r unless _truthy(r) }
    r
  in [:or, *exprs]
    exprs.each { |e| r = scheme_eval(env, e); return r if _truthy(r) }
    false
  in [:set!, var, val]
    env[var] = scheme_eval(env, val)
    nil
  in [:cond]
    nil
  in [:cond, [:else, *body], *]
    body.reduce(nil) { |_, e| scheme_eval(env, e) }
  in [:cond, [test, *exprs], *rest]
    if _branch(scheme_eval(env, test))
      exprs.reduce(nil) { |_, e| scheme_eval(env, e) }
    else
      scheme_eval(env, [:cond, *rest])
    end
  in [Symbol => head, *args]
    evaled = args.map { |e| scheme_eval(env, e) }
    if BUILTINS.key?(head)
      BUILTINS[head].call(*evaled)
    else
      scheme_eval(env, head).call(*evaled)
    end
  in [head, *args]
    evaled = args.map { |e| scheme_eval(env, e) }
    scheme_eval(env, head).call(*evaled)
...
def _get_model
  return {} if $pathcondition.empty?
  return {} unless $solver.check == :sat
  model = {}
  $solver.model.each { |d, v| model[d.to_s] = v.to_s }
  model
end
...
def test(env, exprs)
  ...
  loop do
    ...
    begin
      r = exprs.reduce(nil) { |_, e| scheme_eval(env, e) }
      puts "  result: #{r}"
    rescue DepthError
      puts "  pruned (depth limit)"
    rescue => ex
      model = _get_model
      if model.empty?
        puts "  exception: #{ex}"
      else
        puts "  exception: #{ex}  [#{model.map { |k, v| "#{k}=#{v}" }.join(', ')}]"
      end
    end

    $path.pop while $path.length > 0 && !$path[-1]
    return if $path.empty?
    $path[-1] = false
  end
end

def parse(code) = SXP.read_all(code)
...
def run_benchmark(name, code, **bindings)
  env = BUILTINS.merge(bindings.transform_keys(&:to_sym))
  puts "=== #{name} ==="
  test(env, parse(code))
  puts
end

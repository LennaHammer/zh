...
| 方法  | 核心原理               | 总结                       |
| --- | ------------------ | ------------------------ |
| RCT | 老天爷随机分配，金标准        | 完全相互独立的情况。               |
| OLS | 多放控制变量             | 用控制变量分解掉线性关系。            |
| PSM | 找长得像的人对比           | 根据个体特征从对照组中找最相似的，比如用最近邻。 |
| IV  | 找外生冲击"推一把" X       | Z影响X，但是不影响Y，从而得出X对Y的影响。  |
| DID | 政策时间 + 地区差异，减去时间趋势 | 用未收到政策冲击的个体做对比，减去随时间的变化  |
| RDD | 分数线附近的"准随机"        |                          |
...
面板数据（panel data）= 同一批个体（企业/地区/国家）跨多个时期追踪。核心优势：能同时控制**个体固定效应 + 时间固定效应**，消掉「每个企业特有的、不随时间变的不可观测特征」（这是截面数据无法做到的）。

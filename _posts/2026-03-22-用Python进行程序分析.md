...
```python
# %% [markdown]
# PEERCHECK: A Peer Architecture for Lightweight Symbolic Execution
# Based on Bruni, Disney, Flanagan (2013)
...
#
# __bool__ (BoolProxy):
#   When Python evaluates a conditional (if / while) on a BoolProxy, __bool__ is called.
#   It queries the SMT solver to determine which branches are feasible:
#     1. Only true  feasible → return True  (forced path, no fork)
#     2. Only false feasible → return False (forced path, no fork)
#     3. Both feasible → consult __path__ to pick a branch, record the choice,
#        and append it to __pathcondition__ for subsequent checks.
#   Cases 1&2 are "forced"; case 3 creates a fork recorded in __path__.
#
# test (Model-Checking Loop):
#   Calls the target function repeatedly, each time resetting __pathcondition__
#   while keeping the branch history __path__.  On each run the function follows
#   the choices recorded in __path__.  After the run, backtracks:
#     - pop trailing False entries (backtracked branches)
#     - flip the last True → False (switch to the unexplored else-branch)
#   Stops when __path__ is empty (all paths explored, or depth limit reached).

# %% SMT Solver Interface (§5.1, Figure 4)
# smt_solve — satisfiability check
# (Term constructors are inlined in IntProxy.__init__;
#  arithmetic & comparison use Z3's native operators:
...
# %% Global Path State (§5.3, Figure 6)
# __path__          : branch choices for current execution
# __pathcondition__ : accumulated path constraints
...
# %% BoolProxy (§5.4, Figure 6)
# Wraps an SMT formula; __bool__ drives branch selection
...
class BoolProxy:
    ...
    def __invert__(self):
        return BoolProxy(z3.Not(self.formula))

    def __and__(self, other):
        return BoolProxy(z3.And(self.formula, other.formula))

    def __or__(self, other):
        return BoolProxy(z3.Or(self.formula, other.formula))

    def __bool__(self):
        global __path__, __pathcondition__

        true_cond = smt_solve(z3.And(*(__pathcondition__ + [self.formula])))
        false_cond = smt_solve(z3.And(*(__pathcondition__ + [z3.Not(self.formula)])))
        ...
        if len(__path__) > len(__pathcondition__):
            ...
            __pathcondition__.append(self.formula if branch else z3.Not(self.formula))
            return branch

        if len(__path__) >= 10:
            raise DepthException("Depth exceeded")
...
# %% IntProxy (§5.2, Figure 5)
# Symbolic integer proxy — only special operators need manual defs;
# arithmetic & comparison operators are generated by auto_proxy below.

class IntProxy:
    def __init__(self, term=None):
        if term is None:
            self.term = z3.FreshConst(z3.IntSort())
        elif isinstance(term, int):
            self.term = z3.IntVal(term)
        else:
            self.term = term

    @staticmethod
    def _wrap(other):
        return IntProxy(other) if isinstance(other, int) else other
...
# Auto-generate arithmetic & comparison dunder methods
def auto_proxy(cls):
    for name, (proxy, op) in {
        '__add__': (IntProxy, lambda a, b: a + b),
        '__sub__': (IntProxy, lambda a, b: a - b),
        '__mul__': (IntProxy, lambda a, b: a * b),
        '__eq__': (BoolProxy, lambda a, b: a == b),
        '__lt__': (BoolProxy, lambda a, b: a < b),
        '__le__': (BoolProxy, lambda a, b: a <= b),
        '__gt__': (BoolProxy, lambda a, b: a > b),
        '__ge__': (BoolProxy, lambda a, b: a >= b),
    }.items():
        def f(self, o, proxy=proxy, op=op):
            o = self._wrap(o)
            return proxy(op(self.term, o.term))
        setattr(cls, name, f)

    for name, op in {
        '__neg__': lambda a: -a,
        '__pos__': lambda a: a,
        '__abs__': lambda a: z3.If(a >= 0, a, -a),
    }.items():
        def f(self, op=op):
            return IntProxy(op(self.term))
        setattr(cls, name, f)

    setattr(cls, '__radd__', cls.__add__)
    setattr(cls, '__rmul__', cls.__mul__)
    def rsub(self, o):
        o = self._wrap(o)
        return IntProxy(o.term - self.term)
    setattr(cls, '__rsub__', rsub)
    def ne(self, o):
        o = self._wrap(o)
        return BoolProxy(self.term != o.term)
...
# %% Model Checking Loop (§5.3, Figure 6)
# Calls the target function repeatedly, each time exploring a new path
...
def _save_path():
    return (list(__path__), list(__pathcondition__))

def _restore_path(state):
    __path__[:] = state[0]
    __pathcondition__[:] = state[1]
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                if not condition(*args, **kwargs):
                    raise PreconditionError(f"Precondition failed: {condition}")
            finally:
                _restore_path(saved)
...
def post(condition):
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            saved = _save_path()
            try:
                if not condition(*args, result, **kwargs):
                    raise PostconditionError(f"Postcondition failed: {condition}")
                return result
            finally:
                _restore_path(saved)
        return wrapper
...
def arg(condition):
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            if not condition(*args, **kwargs):
                raise ArgumentError(f"Argument condition failed: {condition}")
    ...
    def class_decorator(cls):
        ...
        def new_init(self, *args, **kwargs):
            ...
            if not condition(self):
                raise InvariantError(f"Invariant failed after __init__")
...
@ensure(lambda x, res: factspec(x) == res)
def faultyfact(x):
    if x == 40:
        return 123456789
    n = 1
...
if __name__ == "__main__":
    ...
    x = IntProxy()
    print("=== Testing succ(x) ===")
    test(succ, x)
    print()

    x = IntProxy()
    print("=== Testing faultyfact(x) ===")
    test(faultyfact, x)
    print()
...
```python
# %% [markdown]
# SCHEME-SYM: Symbolic Execution for Scheme
# Port of PEERCHECK (peercheck.py) using sexpdata for parsing.
# No wrapper classes — Z3 raw types + _branch() for path forking.

# %% Imports
import z3
from sexpdata import Symbol, Quoted, parse
import functools
import operator
...
# %% Core Algorithm
# scheme_eval uses _branch() when an (if ...) condition is a Z3 BoolRef:
# it queries SMT, forks the path, and records constraints.
# The test() loop backtracks over __path__ to explore all feasible paths.
...
def _branch(cond):
    if not isinstance(cond, z3.BoolRef):
        return cond is not False
    global __path__, __pathcondition__
    true_cond = smt_solve(z3.And(*(__pathcondition__ + [cond])))
    false_cond = smt_solve(z3.And(*(__pathcondition__ + [z3.Not(cond)])))
    if true_cond and not false_cond:
        return True
    if false_cond and not true_cond:
        return False
    if len(__path__) > len(__pathcondition__):
        branch = __path__[len(__pathcondition__)]
        __pathcondition__.append(cond if branch else z3.Not(cond))
        return branch
    if len(__path__) >= 10:
        raise DepthException("Depth exceeded")
    __path__.append(True)
    __pathcondition__.append(cond)
    return True
...
# %% Builtin Functions
def _iv(v):
    return v if isinstance(v, z3.ArithRef) else z3.IntVal(v) if isinstance(v, int) else v

def _sub(a, *rest):
    a = _iv(a)
    rest = [_iv(r) for r in rest]
    return -a if not rest else functools.reduce(operator.sub, [a] + rest)
...
BUILTINS = {
    '+': lambda *a: functools.reduce(operator.add, [_iv(x) for x in a]),
    '-': _sub,
    '*': lambda *a: functools.reduce(operator.mul, [_iv(x) for x in a]),
    '=': lambda a, b: operator.eq(_iv(a), _iv(b)),
    '<': lambda a, b: operator.lt(_iv(a), _iv(b)),
    '>': lambda a, b: operator.gt(_iv(a), _iv(b)),
    '<=': lambda a, b: operator.le(_iv(a), _iv(b)),
    '>=': lambda a, b: operator.ge(_iv(a), _iv(b)),
    'cons': lambda a, b: [a] + b if isinstance(b, list) else [a, b],
    'car': lambda lst: lst[0] if lst else None,
    'cdr': lambda lst: lst[1:] if lst else [],
    'null?': lambda lst: isinstance(lst, list) and len(lst) == 0,
    'pair?': lambda lst: isinstance(lst, list) and len(lst) >= 1,
    ...
    'error': _error,
    'not': lambda x: z3.Not(x) if isinstance(x, z3.BoolRef) else x is False,
    'eq?': _eq, 'equal?': _eq,
    'display': lambda x: x,
}
...
# %% Model extractor
def _get_model():
    global __pathcondition__
    if not __pathcondition__:
        return {}
    s = z3.Solver()
    s.add(z3.And(*__pathcondition__))
    if s.check() == z3.sat:
        return s.model()
    return {}

# %% Fresh symbolic integer
def _fresh_int(name=None):
    return z3.Int(name) if name is not None else z3.FreshConst(z3.IntSort())

# %% Lambda (Scheme closure)
class Lambda:
    ...
    def __call__(self, *args):
        ...
        for p, a in zip(self.params, args):
            new_env[p] = a
        return scheme_eval(new_env, self.body)

    def __repr__(self):
        return "#<procedure>"

# %% Scheme Eval
def scheme_eval(env, expr):
    match expr:
        case SYM.T:
            return True
        case SYM.F:
        ...
        case Symbol() as sym:
            if sym.value() in env:
                return env[sym.value()]
            raise NameError(f"Unbound variable: {sym.value()}")
        ...
        case [SYM.IF, cond, then, else_]:
            return scheme_eval(env, then if _branch(scheme_eval(env, cond)) else else_)

        case [SYM.DEFINE, [Symbol() as f, *ps], body]:
            env[f.value()] = Lambda([p.value() for p in ps], body, env)
            return env[f.value()]

        case [SYM.DEFINE, Symbol() as var, val]:
            env[var.value()] = scheme_eval(env, val)
            return env[var.value()]

        case [SYM.DEFINE, lhs, val]:
            k = lhs.value() if isinstance(lhs, Symbol) else lhs
            env[k] = scheme_eval(env, val)
            return env[k]

        case [SYM.LAMBDA, params, body]:
            return Lambda([p.value() for p in params], body, env)

        case [SYM.BEGIN, *exprs]:
            result = None
            for sub in exprs:
                result = scheme_eval(env, sub)
            return result

        case [SYM.LET, bindings, body]:
            new_env = dict(env)
            for b in bindings:
                var = b[0].value() if isinstance(b[0], Symbol) else b[0]
                new_env[var] = scheme_eval(env, b[1])
            return scheme_eval(new_env, body)
        ...
        case [SYM.SET, Symbol() as var, val]:
            env[var.value()] = scheme_eval(env, val)
            return None

        case [SYM.COND, [SYM.ELSE, *body_exprs]]:
            result = None
            for sub in body_exprs:
                result = scheme_eval(env, sub)
            return result

        case [SYM.COND, [test, *exprs], *rest]:
            cond_val = scheme_eval(env, test)
            if _branch(cond_val):
                result = None
                for sub in exprs:
                    result = scheme_eval(env, sub)
                return result
            return scheme_eval(env, [SYM.COND, *rest])

        case [SYM.COND]:
            return None

        case [head, *rest]:
            if isinstance(head, Symbol) and head.value() in BUILTINS:
                return BUILTINS[head.value()](*[scheme_eval(env, e) for e in rest])
            func = scheme_eval(env, head)
            return func(*[scheme_eval(env, e) for e in rest])
...
def test(env, exprs):
    ...
    while True:
        ...
        try:
            ...
            model = _get_model()
            if model:
                bindings = ', '.join(f"{d}={model[d]}" for d in model.decls())
                print(f"  exception: {ex}  [{bindings}]")
            else:
...
BENCH_SUCC = """
(define (succ x)
  (if (= x 42768) (error "fail") (+ x 1)))

(succ x)
"""

BENCH_FACT = """
(define (fact-iter-helper acc m)
  (if (= m 0) acc (fact-iter-helper (* acc m) (- m 1))))

(define (fact-iter n)
  (fact-iter-helper 1 n))

(define (faultyfact x)
  (if (= x 40) 123456789 (fact-iter x)))
...
(quicksort (list x y z))
"""

def run_benchmark(name, code, **bindings):
    env = dict(BUILTINS)
...
# %% Main
if __name__ == "__main__":
    run_benchmark("Testing abs(x)", BENCH_ABS, x=_fresh_int('x'))
    run_benchmark("Testing succ(x)", BENCH_SUCC, x=_fresh_int('x'))
    run_benchmark("Testing faultyfact(x)", BENCH_FACT, x=_fresh_int('x'))
    run_benchmark("Testing quicksort([x, y, z])", BENCH_QSORT,
                  x=_fresh_int('x'), y=_fresh_int('y'), z=_fresh_int('z'))
